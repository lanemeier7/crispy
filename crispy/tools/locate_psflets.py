...
def optimize_coef_from_image(unfiltered, coef, lenslet_ind_x, lenslet_ind_y,
    ...
    for i,frac in enumerate(image_fractions):
        ...
        if show_plots:
            ...
            ax.scatter(_x, _y, s=20, c='r', label='PSFlet Locations', alpha=0.5, edgecolors='none')
            ax.set_title(f'PSFlet Locations After Fit on Central {frac*100:.0f}% of Image')
            ax.legend(loc='upper right',edgecolor='black', facecolor='white', framealpha=1.0)
